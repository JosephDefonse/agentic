...
async def test_complete_chat_contents_function_call_behavior_tool_call(
    ...
    with (
        ...
        response: list[ChatMessageContent] = (
            await chat_completion_base.get_chat_message_contents(
        ...
        assert all(
            isinstance(content, expected_result) for content in response[0].items
        )
...
mock_message_function_call = StreamingChatMessageContent(
    role=AuthorRole.ASSISTANT,
    items=[FunctionCallContent(name="test")],
    choice_index="0",
)
...
async def test_anthropic_sdk_exception(
    kernel: Kernel, mock_settings: AnthropicChatPromptExecutionSettings
):
    client = MagicMock(spec=AsyncAnthropic)
    messages_mock = MagicMock()
...
def test_anthropic_chat_completion_init(anthropic_unit_test_env) -> None:
    ...
    assert (
        anthropic_chat_completion.ai_model_id
        == anthropic_unit_test_env["ANTHROPIC_CHAT_MODEL_ID"]
    )
...
def test_anthropic_chat_completion_init_with_empty_api_key(
...
def test_anthropic_chat_completion_init_with_empty_model_id(
    anthropic_unit_test_env,
) -> None:
    with pytest.raises(ServiceInitializationError):
        AnthropicChatCompletion(
...
def test_prompt_execution_settings_class(anthropic_unit_test_env):
    ...
    prompt_execution_settings = (
        anthropic_chat_completion.get_prompt_execution_settings_class()
    )
    assert prompt_execution_settings == AnthropicChatPromptExecutionSettings


async def test_with_different_execution_settings(
    kernel: Kernel, mock_anthropic_client_completion: MagicMock
):
    chat_history = MagicMock()
    ...
    assert (
        mock_anthropic_client_completion.messages.create.call_args.kwargs["temperature"]
        == 0.2
    )
...
async def test_with_different_execution_settings_stream(
    ...
    assert (
        mock_anthropic_client_completion_stream.messages.stream.call_args.kwargs[
            "temperature"
        ]
        == 0.2
    )
...
async def test_prepare_chat_history_for_request_with_system_message(
    ...
    remaining_messages, system_message_content = (
        chat_completion_base._prepare_chat_history_for_request(
...
async def test_prepare_chat_history_for_request_with_tool_message(
    ...
    remaining_messages, system_message_content = (
        chat_completion_client._prepare_chat_history_for_request(
            chat_history, role_key="role", content_key="content"
        )
    )
    ...
    assert remaining_messages == [
        {"role": AuthorRole.USER, "content": "What is 3+3?"},
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": mock_parallel_tool_calls_message.items[0].text,
                },
                *[
                    {
                        "type": "tool_use",
                    ...
                    for function_call_content in mock_parallel_tool_calls_message.items[
                        1:
                    ]
                ],
            ],
...
async def test_prepare_chat_history_for_request_with_tool_message_right_after_user_message(
    ...
    with pytest.raises(
        ...
        match="Tool message found after a user or system message.",
    ):
        chat_completion_client._prepare_chat_history_for_request(
            chat_history, role_key="role", content_key="content"
        )
...
async def test_prepare_chat_history_for_request_with_tool_message_as_the_first_message(
    ...
    with pytest.raises(
        ServiceInvalidRequestError,
        match="Tool message found without a preceding message.",
    ):
        chat_completion_client._prepare_chat_history_for_request(
            chat_history, role_key="role", content_key="content"
        )
...
def test_chat_completion_reset_settings(
    ...
    settings = AnthropicChatPromptExecutionSettings(
        tools=[{"name": "test"}], tool_choice={"type": "any"}
    )
