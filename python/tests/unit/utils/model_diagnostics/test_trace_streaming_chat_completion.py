...
)  # When no tracer provider is available, the span will be an INVALID_SPAN
async def test_trace_streaming_chat_completion(
    ...
    # Setup
    chat_completion: ChatCompletionClientBase = MockChatCompletion(
        ai_model_id="ai_model_id"
    )
    ...
    with patch.object(
        ...
        MockChatCompletion._inner_get_streaming_chat_message_contents = (
            trace_streaming_chat_completion(MockChatCompletion.MODEL_PROVIDER_NAME)(
        ...
        mock_span.set_attribute.assert_any_call(
            gen_ai_attributes.ADDRESS, chat_completion.service_url()
        )

        # No all connectors take the same parameters
        if execution_settings.extension_data.get("max_completion_tokens") is not None:
            mock_span.set_attribute.assert_any_call(
                gen_ai_attributes.max_completion_tokens,
                execution_settings.extension_data["max_completion_tokens"],
            )
        if execution_settings.extension_data.get("temperature") is not None:
            mock_span.set_attribute.assert_any_call(
                gen_ai_attributes.TEMPERATURE,
                execution_settings.extension_data["temperature"],
            )
        if execution_settings.extension_data.get("top_p") is not None:
            mock_span.set_attribute.assert_any_call(
                gen_ai_attributes.TOP_P, execution_settings.extension_data["top_p"]
            )
        ...
        mock_logger.info.assert_has_calls(
            [
                call(
                    ...
                    extra={
                        gen_ai_attributes.EVENT_NAME: gen_ai_attributes.ROLE_EVENT_MAP.get(
                            message.role
                        ),
                        gen_ai_attributes.SYSTEM: MockChatCompletion.MODEL_PROVIDER_NAME,
        ...
        if mock_response[0].metadata.get("id") is not None:
            mock_span.set_attribute.assert_any_call(
                gen_ai_attributes.RESPONSE_ID, mock_response[0].metadata["id"]
            )
        if any(completion.finish_reason is not None for completion in mock_response):
            mock_span.set_attribute.assert_any_call(
                gen_ai_attributes.FINISH_REASON,
                ",".join(
                    [str(completion.finish_reason) for completion in mock_response]
                ),
            )
...
async def test_trace_streaming_chat_completion_exception(
    ...
    with patch.object(
        ...
        with pytest.raises(ServiceResponseException):
            async for (
                update
            ) in chat_completion._inner_get_streaming_chat_message_contents(
                chat_history, execution_settings
            ):
                pass
        ...
        mock_span.set_attribute.assert_any_call(
            gen_ai_attributes.ERROR_TYPE, str(type(exception))
        )
