...
class Agenda:
    ...
    async def update_agenda(
        ...
        while True:
            try:
                ...
                self.logger.info(
                    f"Agenda updated successfully: {self.get_agenda_for_prompt()}"
                )
                ...
                if len(previous_attempts) > self.max_agenda_retries:
                    self.logger.warning(
                        f"Failed to update agenda after {self.max_agenda_retries} attempts."
                    )
                    return PluginOutput(False, [])
                else:
                    self.logger.info(
                        f"Attempting to fix the agenda error. Attempt {len(previous_attempts)}."
                    )
                    response = await self._fix_agenda_error(
                        llm_formatted_attempts, conversation
                    )
    ...
    def get_agenda_for_prompt(self) -> str:
        ...
        total_resource = format_resource(
            sum([item["resource"] for item in agenda_items]),
    ...
    def update_agenda_items(
        ...
        items: Annotated[
            list[Items],
            "Ordered list of items to be completed in the remainder of the conversation",
        ],
    ):
        pass

    async def _fix_agenda_error(
        self, previous_attempts: str, conversation: Conversation
    ) -> None:
    ...
    def _validate_agenda_update(
        self, items: list[dict[str, str]], remaining_turns: int
    ) -> None:
        ...
        if (self.resource_constraint_mode == ResourceConstraintMode.MAXIMUM) and (
            total_resources > remaining_turns
        ):
            total_resource_instruction = f"The total turns allocated in the agenda must not exceed the remaining amount ({remaining_turns})"
            violations.append(
                f"{total_resource_instruction}; but the current total is {total_resources}."
            )

        # In exact mode if the total resources were not exactly equal to the remaining turns
        if (self.resource_constraint_mode == ResourceConstraintMode.EXACT) and (
            total_resources != remaining_turns
        ):
            total_resource_instruction = f"The total turns allocated in the agenda must equal the remaining amount ({remaining_turns})"
            violations.append(
                f"{total_resource_instruction}; but the current total is {total_resources}."
            )
        ...
        if len(violations) > 0:
            self.logger.debug(
                f"Agenda update failed due to the following violations: {violations}."
            )
