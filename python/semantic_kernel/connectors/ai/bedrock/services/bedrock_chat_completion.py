...
class BedrockChatCompletion(BedrockBase, ChatCompletionClientBase):
    ...
    async def _inner_get_streaming_chat_message_contents(
        ...
        for event in response_stream.get("stream"):
            if "messageStart" in event:
                ...
                yield [
                    self._parse_content_block_delta_event(
                        event, function_invoke_attempt
                    )
                ]
                ...
                raise ServiceInvalidResponseError(
                    f"Unknown event type in the response: {event}"
                )
    ...
    def _prepare_settings_for_request(
        ...
        prepared_settings = {
            ...
            "inferenceConfig": remove_none_recursively(
                {
                    "maxTokens": settings.max_completion_tokens,
    ...
    def _create_chat_message_content(
        ...
        finish_reason: FinishReason | None = (
            finish_reason_from_bedrock_to_semantic_kernel(response["stopReason"])
        )
        ...
        for content in response["output"]["message"]["content"]:
            if "text" in content:
                ...
                raise ServiceInvalidResponseError(
                    f"Unsupported content type in the response: {content}"
                )
    ...
    def _parse_message_start_event(
    ...
    def _parse_content_block_start_event(
        self, event: dict[str, Any]
    ) -> StreamingChatMessageContent:
    ...
    def _parse_message_stop_event(
        ...
        return StreamingChatMessageContent(
            ...
            choice_index=0,
            inner_content=event,
            finish_reason=finish_reason_from_bedrock_to_semantic_kernel(
                event["messageStop"]["stopReason"]
            ),
            metadata=metadata,
        )

    def _parse_metadata_event(
        self, event: dict[str, Any]
    ) -> StreamingChatMessageContent:
