...
class NexusRavenCompletion(TextCompletionClientBase, ChatCompletionClientBase):
    ...
    async def get_chat_message_contents(
        ...
        **kwargs: Any,
        ...
        for part in result[0].text.split(";"):
            try:
                function_call, function_result = await self._execute_function_calls(
                    part, chat_history, **kwargs
                )
                if function_call:
                    messages.extend(
                        [
                    ...
                    messages.append(
                        ChatMessageContent(
                            role="assistant", content=part, ai_model_id=self.ai_model_id
                        )
                    )
            ...
            await asyncio.gather(
                *[
                    self._execute_function_call(
                        call, chat_history, kwargs.get("kernel")
                    )
                ...
                if call["result"] is None:
                    for name, arg in call["args"].items():
                        if (
                            ...
                            and call_stack[arg[0]]["result"] is not None
                        ):
                            function_result: FunctionResultContent = call_stack[arg[0]][
                                "result"
                            ]
    ...
    async def _execute_function_call(
        ...
        call_def["fcc"] = FunctionCallContent(
            name=call_def["func"],
            arguments=json.dumps(call_def["args"]),
            id=str(call_def["idx"]),
        )
    ...
    async def get_text_contents(
        self, prompt: str, settings: NexusRavenPromptExecutionSettings
    ) -> list[TextContent]:
        result = await self.client.text_generation(
            prompt, **settings.prepare_settings_dict(), stream=False
        )
...
def format_functions_for_prompt():
    ...
    for func in functions:
        ...
        func_string = (
            f"{func.fully_qualified_name.replace('-', '_')}({', '.join(args_strings)})"
        )
...
kernel.add_service(
    NexusRavenCompletion(
        service_id="nexus",
        ai_model_id="raven",
        endpoint_url="http://nexusraven.nexusflow.ai",
    )
)
...
async def run_question(user_input: str, chat_history: ChatHistory):
    ...
    result = await kernel.invoke(
        plugin_name="kernel", function_name="function_call", arguments=arguments
    )
    chat_history.add_user_message(user_input)
    for msg in result.value:
        chat_history.add_message(msg)
    final_result = await kernel.invoke(
        plugin_name="kernel", function_name="chat", arguments=arguments
    )
...
async def main():
    chat_history = ChatHistory()
    user_input_example = "my cake is 3 centimers high and 20 centimers in radius, can you subtract 200 from that number?"
