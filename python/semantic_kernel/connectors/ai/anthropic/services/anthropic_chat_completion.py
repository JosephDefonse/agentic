...
from semantic_kernel.connectors.ai.anthropic.settings.anthropic_settings import (
    AnthropicSettings,
)
...
from semantic_kernel.contents.streaming_chat_message_content import (
    STREAMING_CMC_ITEM_TYPES as STREAMING_ITEM_TYPES,
)
from semantic_kernel.contents.streaming_chat_message_content import (
...
ANTHROPIC_TO_SEMANTIC_KERNEL_FINISH_REASON_MAP = {
    ...
    "max_completion_tokens": SemanticKernelFinishReason.LENGTH,
...
class AnthropicChatCompletion(ChatCompletionClientBase):
    ...
    @override
    def _reset_function_choice_settings(
    ...
    async def _inner_get_chat_message_contents(
        ...
        settings.messages, parsed_system_message = (
    ...
    async def _inner_get_streaming_chat_message_contents(
        ...
        settings.messages, parsed_system_message = (
            self._prepare_chat_history_for_request(chat_history, stream=True)
        )
        settings.ai_model_id = settings.ai_model_id or self.ai_model_id
        if settings.system is None and parsed_system_message is not None:
            settings.system = parsed_system_message
    ...
    def _prepare_chat_history_for_request(
        ...
        for i in range(len(chat_history)):
            ...
            if curr_message.role == AuthorRole.SYSTEM:
                ...
                curr_message.role == AuthorRole.USER
                or curr_message.role == AuthorRole.ASSISTANT
            ):
                formatted_messages.append(
                    MESSAGE_CONVERTERS[curr_message.role](curr_message)
                )
                ...
                if (
                    prev_message.role == AuthorRole.USER
                    or prev_message.role == AuthorRole.SYSTEM
                ):
                ...
                if prev_message.role == AuthorRole.ASSISTANT:
                    ...
                    formatted_messages[-1][content_key] += formatted_message[
                        content_key
                    ]
            else:
                raise ServiceInvalidRequestError(
                    f"Unsupported role in chat history: {curr_message.role}"
                )
    ...
    def _create_chat_message_content(
        ...
        if response.stop_reason:
            finish_reason = ANTHROPIC_TO_SEMANTIC_KERNEL_FINISH_REASON_MAP[
        ...
        if isinstance(stream_event, TextEvent):
            ...
            items.append(
                FunctionCallContent(
                    ...
                    arguments=(
                        json.dumps(tool_use_block.input)
                        if tool_use_block.input
                        else None
                    ),
                )
            )
            ...
            finish_reason = ANTHROPIC_TO_SEMANTIC_KERNEL_FINISH_REASON_MAP[
                str(stream_event.delta.stop_reason)
            ]
                ...
                **settings.prepare_settings_dict()
                ...
                async for stream_event in stream:
                    if isinstance(stream_event, RawMessageStartEvent):
                        metadata["usage"][
                            "input_tokens"
                        ] = stream_event.message.usage.input_tokens
                        metadata["id"] = stream_event.message.id
                    elif isinstance(
                        stream_event, (TextEvent, RawMessageDeltaEvent)
                    ) or (
                        isinstance(stream_event, ContentBlockStopEvent)
                        and stream_event.content_block.type == "tool_use"
                    ):
                        yield [
                            self._create_streaming_chat_message_content(
                                stream_event, metadata, function_invoke_attempt
                            )
                        ]
        except Exception as ex:
            raise ServiceResponseException(
    ...
    def _get_tool_calls_from_message(
