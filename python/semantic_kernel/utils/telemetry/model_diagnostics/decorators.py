...
from semantic_kernel.utils.telemetry.model_diagnostics.model_diagnostics_settings import (
    ...
    def inner_trace_text_completion(completion_func: Callable) -> Callable:
        ...
        async def wrapper_decorator(*args: Any, **kwargs: Any) -> list[TextContent]:
            ...
            settings: "PromptExecutionSettings" = (
                kwargs["settings"] if kwargs.get("settings") is not None else args[2]
            )
            ...
            with use_span(
                ...
                try:
                    completions: list[TextContent] = await completion_func(
                        *args, **kwargs
                    )
                    _set_completion_response(current_span, completions, model_provider)
                    return completions
        ...
        async def wrapper_decorator(
            *args: Any, **kwargs: Any
            ...
            with use_span(
                ...
                _set_completion_input(model_provider, prompt)
                try:
                    async for streaming_text_contents in completion_func(
                        *args, **kwargs
                    ):
                        for streaming_text_content in streaming_text_contents:
                            ...
                            all_text_contents[choice_index].append(
                    ...
                    all_text_contents_flattened = [
                        reduce(lambda x, y: x + y, messages)
                        for messages in all_text_contents.values()
                    ]
                    _set_completion_response(
                        current_span, all_text_contents_flattened, model_provider
                    )
                except Exception as exception:
                    _set_completion_error(current_span, exception)
                    raise
...
def _get_completion_span(
    ...
    if execution_settings:
        attribute_name_map = {
            ...
            "max_completion_tokens": gen_ai_attributes.max_completion_tokens,
...
def _set_completion_response(
    ...
    if finish_reasons:
        current_span.set_attribute(
            gen_ai_attributes.FINISH_REASON, ",".join(finish_reasons)
        )
    ...
    if isinstance(usage, CompletionUsage):
        if usage.prompt_tokens:
            current_span.set_attribute(
                gen_ai_attributes.INPUT_TOKENS, usage.prompt_tokens
            )
        if usage.completion_tokens:
            current_span.set_attribute(
                gen_ai_attributes.OUTPUT_TOKENS, usage.completion_tokens
            )
