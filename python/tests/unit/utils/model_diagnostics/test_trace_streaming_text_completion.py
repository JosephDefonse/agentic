...
async def test_trace_streaming_text_completion(
    ...
    # Setup
    text_completion: TextCompletionClientBase = MockTextCompletion(
        ai_model_id="ai_model_id"
    )
    ...
    with patch.object(
        MockTextCompletion, "_inner_get_streaming_text_contents", return_value=iterable
    ):
        # We need to reapply the decorator to the method since the mock will not have the decorator applied
        MockTextCompletion._inner_get_streaming_text_contents = (
            trace_streaming_text_completion(MockTextCompletion.MODEL_PROVIDER_NAME)(
                text_completion._inner_get_streaming_text_contents
            )
        )

        updates = []
        async for update in text_completion.get_streaming_text_contents(
