    ...
    with (
        ...
        await kernel.invoke_function_call(
            ...
            function_behavior=FunctionChoiceBehavior.Auto(
                filters={"included_functions": ["function"]}
            ),
        )
...
async def test_kernel_invoke_deep_copy_preserves_previous_state():
    ...
    func_call = FunctionCallContent(
        plugin_name="lights",
        function_name="get_lights_states",
        name="get_lights_states",
        arguments={},
    )
...
async def test_invoke_function_call_non_allowed_func_throws(
    ...
    with patch("semantic_kernel.kernel.logger", autospec=True):
        await kernel.invoke_function_call(
            ...
            request_index=0,
            function_behavior=FunctionChoiceBehavior.Auto(
                filters={"included_functions": ["unknown"]}
            ),
        )
...
async def test_invoke_function_call_with_missing_or_unexpected_args(kernel: Kernel):
    ...
    func_meta = KernelFunctionMetadata(
        name="function", is_prompt=False, parameters=[required_param]
    )
    func_mock = MagicMock(spec=KernelFunction)
    func_mock.metadata = func_meta
    func_mock.name = "function"
...
def test_add_plugin_from_directory_with_encoding(kernel: Kernel):
    ...
    "input_variables": [
        ...
        plugin = kernel.add_plugin(
            ...
            plugin_name="test_encoding_plugin",
            encoding="utf-8",
        )
...
def test_experimental_class_has_decorator_and_flag(experimental_plugin_class):
    ...
    assert (
        "This class is marked as 'experimental' and may change in the future"
...
def test_kernel_model_dump(
    ...
    kernel.add_plugin(custom_plugin_class(), "TestPlugin")
    kernel.add_filter(
        FilterTypes.AUTO_FUNCTION_INVOCATION, auto_function_invocation_filter
    )
...
def test_kernel_clone(
    ...
    kernel_clone.plugins["TestPlugin"].functions[
        "getLightStatus"
    ].metadata.name = "getLightStatus2"
    assert (
        kernel.plugins["TestPlugin"].functions["getLightStatus"].metadata.name
        == "getLightStatus"
    )

    kernel_clone.plugins.clear()
    kernel_clone.remove_filter(
        filter_type=FilterTypes.AUTO_FUNCTION_INVOCATION, position=0
    )
    kernel_clone.remove_all_services()

    assert kernel.plugins is not None and len(kernel.plugins) > 0
    assert (
        kernel.auto_function_invocation_filters is not None
        and len(kernel.auto_function_invocation_filters) > 0
    )
