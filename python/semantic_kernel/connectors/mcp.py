...
def _mcp_content_types_to_kernel_content(
    ...
    if isinstance(mcp_type, types.ImageContent):
        return ImageContent(
            data=mcp_type.data, mime_type=mcp_type.mimeType, inner_content=mcp_type
        )
...
def _kernel_content_to_mcp_content_types(
    ...
    if isinstance(content, ImageContent):
        return [
            types.ImageContent(
                type="image", data=content.data_string, mimeType=content.mime_type
            )
        ]
    if isinstance(content, AudioContent):
        return [
            types.AudioContent(
                type="audio", data=content.data_string, mimeType=content.mime_type
            )
        ]
        ...
        return [
            types.EmbeddedResource(
                ...
                resource=types.BlobResourceContents(
                    blob=content.data_string,
                    mimeType=content.mime_type,
                    uri=content.uri or "sk://binary",
                ),
            )
        ]
    if isinstance(content, ChatMessageContent):
        ...
        for item in content.items:
            if isinstance(
                item, (TextContent, ImageContent, BinaryContent, AudioContent)
            ):
...
class MCPPluginBase:
    ...
    async def sampling_callback(
        ...
        elif "max_completion_tokens" in completion_settings.__class__.model_fields:
            completion_settings.max_completion_tokens = params.maxTokens  # type: ignore
    ...
    async def load_prompts(self):
        ...
        for prompt in prompt_list.prompts if prompt_list else []:
            ...
            func.__kernel_function_parameters__ = _get_parameter_dict_from_mcp_prompt(
                prompt
            )
    ...
    async def load_tools(self):
        ...
        for tool in tool_list.tools if tool_list else []:
            ...
            func = kernel_function(name=local_name, description=tool.description)(
                partial(self.call_tool, tool.name)
            )
            func.__kernel_function_parameters__ = _get_parameter_dicts_from_mcp_tool(
                tool
            )
            setattr(self, local_name, func)
    ...
    async def call_tool(
        ...
        try:
            return _mcp_call_tool_result_to_kernel_contents(
    ...
    async def get_prompt(
        ...
        try:
            ...
            return [
                _mcp_prompt_message_to_kernel_content(message)
            ...
            raise FunctionExecutionException(
                f"Failed to call prompt '{prompt_name}'."
            ) from ex
...
def create_mcp_server_from_kernel(
    ...
    **kwargs: Any,
    ...
    functions_to_expose = [
        ...
        if func.name not in (excluded_functions or [])
    ]
                ...
                types.Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            ...
                            if param.name
                            and param.schema_data
                            and param.include_in_function_choices
                        },
                        "required": [
                            param.name
                            for param in func.parameters
                    ...
                    for item in value:
                        match item:
                            case (
                                ...
                                messages.extend(
                                    _kernel_content_to_mcp_content_types(item)
                                )
                            case _:
                                messages.append(
    ...
    if prompts:
        ...
        async def _get_prompt(
            ...
            prompt = next(
                (p for p in prompts if p.prompt_template_config.name == name), None
            )
            if prompt is None:
                return types.GetPromptResult(
                    description="Prompt not found", messages=[]
                )
    ...
    async def _call_kernel_function(
        function_name: str, arguments: Any
    ) -> FunctionResult | None:
