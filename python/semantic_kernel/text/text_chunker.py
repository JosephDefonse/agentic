...
def split_plaintext_paragraph(
    ...
    for line in text:
        split_lines.extend(
            _split_text_lines(
                text=line,
                max_token_per_line=max_completion_tokens,
                trim=True,
                token_counter=token_counter,
            )
        )
...
def _split_text_paragraph(
    ...
    for line in text:
        ...
        if (
            num_tokens_paragraph + num_tokens_line + 1 >= max_completion_tokens
    ...
    if len(paragraphs) > 1:
        ...
        if token_counter(last_para) < max_completion_tokens / 4:
            ...
            if (
                last_para_token_count + sec_last_para_token_count
...
def _split_markdown_lines(
    ...
    return _split_str_lines(
        text=text,
        max_completion_tokens=max_token_per_line,
...
def _split_str(
    ...
    if token_counter(text) <= max_completion_tokens:
