...
pytestmark = pytest.mark.parametrize(
    ...
    [
            ...
            [
                [
                    ChatMessageContent(
                        ...
                        items=[
                            TextContent(
                                text="You're very bad at math. Don't attempt to do it yourself."
                            )
                        ],
                    ),
                    ChatMessageContent(
            ...
            [
                [
                    ChatMessageContent(
                        role=AuthorRole.USER,
                        items=[
                            TextContent(text="Find the person whose id is 9b3f6e40.")
                        ],
                    ),
                ]
            ],
        ...
        pytest.param(
            ...
            {
                "function_choice_behavior": FunctionChoiceBehavior.Auto(
                    filters={"excluded_plugins": ["task_plugin"]}
                )
            },
            [
                [
            ...
            [
                [
                    ...
                    ChatMessageContent(
                        ...
                        items=[
                            FunctionResultContent(
                                id="123456789", name="finance-search", result="1.2B"
                            )
                        ],
                    ),
                ],
        ...
        pytest.param(
            ...
            marks=[
                pytest.mark.skipif(
                    not google_ai_setup,
                    reason="Google AI Environment Variables not set",
                ),
                pytest.mark.xfail(
        ...
        pytest.param(
            ...
            marks=(
                pytest.mark.skipif(
                    not ollama_tool_call_setup, reason="Need local Ollama setup"
                ),
                pytest.mark.ollama,
            ),
...
class TestChatCompletionWithFunctionCalling(ChatCompletionTestBase):
    ...
    def _evaluate_auto_function_choice(
        ...
        assert any(
            ...
            for item in chat_history.messages[skip_counts].items
        )
        # Expect the second message to be a FunctionResultContent
        assert isinstance(
            chat_history.messages[skip_counts + 1].items[0], FunctionResultContent
        )
        # Expect the third message to be a TextContent
        assert any(
            isinstance(item, TextContent)
            for item in chat_history.messages[skip_counts + 2].items
        )
    ...
    async def _test_helper(
        ...
        assert (
            len(inputs) == 1
        ), "Invalid parameterization: Only one input message or a single list are allowed"
        history = ChatHistory()
