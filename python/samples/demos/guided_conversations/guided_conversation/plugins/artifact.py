...
class Artifact:
    ...
    async def update_artifact(
        self, field_name: str, field_value: Any, conversation: Conversation
    ) -> PluginOutput:
        ...
        while True:
            try:
                ...
                self.logger.warning(
                    f"Error updating field {field_name}: {e}. Retrying..."
                )
                ...
                # This is the case where the agent has decided to resume the conversation.
                    ...
                    return PluginOutput(True, conversation_messages)
                self.logger.warning(
                    f"Agent failed to fix field {field_name}. Retrying..."
                )
    ...
    def get_artifact_for_prompt(self) -> str:
        ...
        return {
            k: v
            for k, v in self.artifact.model_dump().items()
            if k not in failed_fields
        }
    ...
    def get_schema_for_prompt(self, filter_one_field: str | None = None) -> str:
        ...
        if filter_one_field:
            ...
            filtered_schema = {
                "properties": {
                    filter_one_field: self.original_schema["properties"][
                        filter_one_field
                    ]
                }
            }
...
Remember that when updating the artifact, the field will be the original field name in the artifact and the JSON object(s) will be the value."""
    ...
    def _get_type_if_subtype(
        self, target_type: type[Any], base_type: type[Any]
    ) -> type[Any] | None:
    ...
    def _modify_classes(
        self, artifact_class: BaseModel
    ) -> dict[str, type[BaseModelLLM]]:
    ...
    def _replace_type_annotations(
        self,
        field_annotation: type[Any] | None,
        modified_classes: dict[str, type[BaseModelLLM]],
    ) -> type:
        ...
        if origin is None:
            ...
            new_args = tuple(
                self._replace_type_annotations(arg, modified_classes) for arg in args
            )
            return origin[new_args]

    def _modify_base_artifact(
        self,
        artifact_model: type[BaseModelLLM],
        ...
        artifact_model = create_model(
            "Artifact", __base__=BaseModelLLM, **field_definitions
        )
    ...
    def _is_valid_field(self, field_name: str) -> tuple[bool, ChatMessageContent]:
        ...
        if field_name not in self.artifact.model_fields:
            error_message = (
                f'Field "{field_name}" is not a valid field in the artifact.'
            )
    ...
    async def _fix_artifact_error(
        ...
        req_settings.max_completion_tokens = 2000

        self.kernel.add_function(
            plugin_name=self.id, function=self.update_artifact_field
        )
        ...
        req_settings.function_choice_behavior = FunctionChoiceBehavior.Auto(
            auto_invoke=False, filters=filter
        )
