...
def remove_none_recursively(data: dict, max_depth: int = 5) -> dict:
    ...
    return {
        k: remove_none_recursively(v, max_depth=max_depth - 1)
...
def _format_assistant_message(message: ChatMessageContent) -> dict[str, Any]:
    ...
    for item in message.items:
        ...
        if isinstance(item, TextContent):
            ...
            raise ServiceInvalidRequestError(
                f"Unsupported content type in an assistant message: {type(item)}"
            )
...
def _format_tool_message(message: ChatMessageContent) -> dict[str, Any]:
    ...
    for item in message.items:
        ...
        if isinstance(item, TextContent):
            ...
            raise ServiceInvalidRequestError(
                f"Unsupported content type in a tool message: {type(item)}"
            )
        ...
        if type == FunctionChoiceType.AUTO:
            ...
            if len(function_choice_configuration.available_functions) > 1:
                ...
                settings.tool_choice = {
                    "tool": {
                        "name": function_choice_configuration.available_functions[
                            0
                ...
                "toolSpec": {
                    ...
                    "inputSchema": {
                        "json": {
                            ...
                            "required": [
                                p.name for p in function.parameters if p.is_required
                            ],
                        }
                    },
                }
...
def finish_reason_from_bedrock_to_semantic_kernel(
    finish_reason: str,
) -> FinishReason | None:
