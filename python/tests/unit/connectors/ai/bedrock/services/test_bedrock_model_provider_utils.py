...
def test_update_settings_from_function_choice_configuration_auto(
...
def test_update_settings_from_function_choice_configuration_auto_without_plugin(
    kernel: Kernel,
) -> None:
    settings = BedrockChatPromptExecutionSettings()
...
def test_update_settings_from_function_choice_configuration_none(
    kernel: Kernel,
) -> None:
    settings = BedrockChatPromptExecutionSettings()
...
def test_update_settings_from_function_choice_configuration_required_with_more_than_one_functions(
    ...
    kernel.add_plugin(plugin=custom_plugin_class(), plugin_name="custom_plugin")
    kernel.add_plugin(
        plugin=experimental_plugin_class(), plugin_name="experimental_plugin"
    )
...
def test_inference_profile_with_bedrock_model() -> None:
    ...
    assert (
        BedrockModelProvider.to_model_provider(us_amazon_inference_profile)
        == BedrockModelProvider.AMAZON
    )

    us_anthropic_inference_profile = "us.anthropic.claude-3-sonnet-20240229-v1:0"
    assert (
        BedrockModelProvider.to_model_provider(us_anthropic_inference_profile)
        == BedrockModelProvider.ANTHROPIC
    )

    eu_meta_inference_profile = "eu.meta.llama3-2-3b-instruct-v1:0"
    assert (
        BedrockModelProvider.to_model_provider(eu_meta_inference_profile)
    ...
    with pytest.raises(
        ValueError,
        match="Model ID unknown does not contain a valid model provider name.",
    ):
...
def test_format_user_message_unsupported_content() -> None:
    ...
    func_call_item = FunctionCallContent(
        id="123", function_name="test_function", arguments="{}"
    )
    ...
    assert "Only text and image content are supported in a user message." in str(
        exc.value
    )
...
def test_format_assistant_message_unsupported_type() -> None:
    ...
    func_res_item = FunctionResultContent(
        id="res1", function_name="some_function", result="some_result"
    )
    assistant_message = ChatMessageContent(
        role=AuthorRole.ASSISTANT, items=[func_res_item]
    )
...
def test_format_tool_message_text() -> None:
    ...
    assert (
        formatted["role"] == "user"
    )  # note that for a tool message, role set to 'user'
...
def test_format_tool_message_function_result() -> None:
    ...
    func_result_item = FunctionResultContent(
        id="res_id", function_name="test_function", result="some result"
    )
...
def test_finish_reason_from_bedrock_to_semantic_kernel_length() -> None:
    """Test that 'max_completion_tokens' maps to FinishReason.LENGTH"""
    reason = finish_reason_from_bedrock_to_semantic_kernel("max_completion_tokens")
...
def test_update_settings_from_function_choice_configuration_required_one(
    mock_bedrock_settings,
) -> None:
    ...
    update_settings_from_function_choice_configuration(
        config, mock_bedrock_settings, FunctionChoiceType.REQUIRED
    )
