...
def test_anthropic_chat_prompt_execution_settings_from_custom_completion_config():
    ...
    assert chat_settings.max_completion_tokens == 128
...
def test_create_options():
    ...
    assert options["max_completion_tokens"] == 128


def test_tool_choice_none():
    with pytest.raises(
        ServiceInvalidExecutionSettingsError,
        match="Tool choice 'none' is not supported by Anthropic.",
    ):
        AnthropicChatPromptExecutionSettings(
            service_id="test_service",
            extension_data={
