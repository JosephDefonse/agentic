...
def test_instantiate_prompt_execution_settings():
    ...
    settings = service.instantiate_prompt_execution_settings(
        max_completion_tokens=25, temperature=0.7, top_p=0.5
    )
    assert settings.extension_data["max_completion_tokens"] == 25
...
def test_get_prompt_execution_settings_from_settings():
    ...
    settings = PromptExecutionSettings(
        service_id="service",
        extension_data={"max_completion_tokens": 25, "temperature": 0.7, "top_p": 0.5},
    )
    new_settings = service.get_prompt_execution_settings_from_settings(settings)
    assert new_settings.extension_data["max_completion_tokens"] == 25
