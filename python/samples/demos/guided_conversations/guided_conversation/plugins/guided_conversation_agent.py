...
from guided_conversation.functions.execution import (
    end_conversation,
    execution,
    send_message,
)
...
from guided_conversation.utils.plugin_helpers import (
    PluginOutput,
    format_kernel_functions_as_tools,
)
...
class GuidedConversation:
    def __init__(
        ...
        self.agenda = Agenda(
            self.kernel,
            self.service_id,
            self.resource.get_resource_mode(),
        ...
        self.req_settings = self.kernel.get_prompt_execution_settings_from_service_id(
            self.service_id
        )
        self.req_settings.max_completion_tokens = 2000
        self.kernel.add_function(
            plugin_name=ToolName.SEND_MSG_TOOL.value, function=send_message
        )
        self.kernel.add_function(
            plugin_name=ToolName.END_CONV_TOOL.value, function=end_conversation
        )
        self.kernel.add_function(
            plugin_name=ToolName.UPDATE_ARTIFACT_TOOL.value,
            function=self.artifact.update_artifact_field,
        )
        self.kernel.add_function(
            plugin_name=ToolName.UPDATE_AGENDA_TOOL.value,
            function=self.agenda.update_agenda_items,
        )
        ...
        self.kernel_function_execute_plan = self.kernel.add_function(
            plugin_name="gc_agent", function=self.execute_plan
        )
        self.kernel_function_final_update = self.kernel.add_function(
            plugin_name="gc_agent", function=self.final_update
        )
    ...
    async def step_conversation(self, user_input: str | None = None) -> GCOutput:
        ...
        while self.current_failed_decision_attempts < MAX_DECISION_RETRIES:
            ...
            for plugin_name, plugin_args in plugins:
                if (
                    plugin_name
                    == f"{ToolName.UPDATE_ARTIFACT_TOOL.value}-{ToolName.UPDATE_ARTIFACT_TOOL.value}"
                ):
                    ...
                    plugin_name
                    == f"{ToolName.UPDATE_AGENDA_TOOL.value}-{ToolName.UPDATE_AGENDA_TOOL.value}"
                ):
            ...
            if terminal_plugins:
                ...
                if (
                    plugin_name
                    == f"{ToolName.SEND_MSG_TOOL.value}-{ToolName.SEND_MSG_TOOL.value}"
                ):
                    gc_output.ai_message = plugin_args["message"]
                elif (
                    plugin_name
                    == f"{ToolName.END_CONV_TOOL.value}-{ToolName.END_CONV_TOOL.value}"
                ):
        ...
        self.logger.warning(
            f"Failed to execute plan after {MAX_DECISION_RETRIES} attempts."
        )
        self.resource.increment_resource()
        gc_output = GCOutput()
        gc_output.ai_message = (
            "An error occurred and I must sadly end the conversation."
        )
        gc_output.is_conversation_over = True
        return gc_output
    ...
    async def final_update(self):
        ...
        self.logger.info(
            "Final update of the artifact prior to terminating the conversation."
        )
        ...
        if validation_result != ToolValidationResult.SUCCESS:
            self.logger.warning(
                f"No artifact change during final update due to: {validation_result.value}"
            )
            pass
        else:
            for i in range(len(parsed_result["tool_names"])):
                ...
                if (
                    ...
                    if plugin_output.update_successful:
                        self.logger.info(
                            f"Artifact field {tool_args['field_name']} successfully updated."
                        )
                        # Set turn numbers
                        ...
                        self.logger.error(
                            f"Final artifact field update of {tool_args['field_name']} failed."
                        )
