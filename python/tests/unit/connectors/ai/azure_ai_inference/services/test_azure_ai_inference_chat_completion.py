...
def test_azure_ai_inference_chat_completion_init(
    ...
    azure_ai_inference = AzureAIInferenceChatCompletion(
        model_id, instruction_role="developer"
    )
...
async def test_azure_ai_inference_response_format_json_schema(
    ...
    _ = await azure_ai_inference_service.get_chat_message_contents(
        chat_history=chat_history, settings=settings
    )
...
async def test_azure_ai_inference_streaming_chat_completion_with_function_choice_behavior(
    ...
    assert (
        all_messages[0].role == "assistant"
    ), f"Unexpected role for first message: {all_messages[0].role}"
    assert (
        all_messages[0].content == ""
    ), f"Unexpected content for first message: {all_messages[0].content}"
    assert (
        all_messages[0].finish_reason == FinishReason.TOOL_CALLS
    ), f"Unexpected finish reason for first message: {all_messages[0].finish_reason}"

    # Validate the second message
    assert (
        all_messages[1].role == "tool"
    ), f"Unexpected role for second message: {all_messages[1].role}"
    assert (
        all_messages[1].content == ""
    ), f"Unexpected content for second message: {all_messages[1].content}"
...
async def test_azure_ai_inference_streaming_response_format_json_schema(
    ...
    messages = []
    async for chunk in azure_ai_inference_service.get_streaming_chat_message_contents(
        chat_history, settings
    ):
